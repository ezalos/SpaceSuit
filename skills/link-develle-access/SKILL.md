...
| WARNING | `CLOUDFLARE_ACCESS_TOKEN_GCLOUD_AUTH` missing from env | `link-develle-access: access token not in env; not vaulted yet, needs to be in env or ~/42/GroundControl/network/domains/.envrc as plaintext (secrets check there)` |
...
```bash
cd ~/42/GroundControl/network/domains && eval "$(grep -E '^export (CLOUDFLARE_ACCESS_TOKEN_GCLOUD_AUTH|CLOUDFLARE_ACCOUNT_ID)=' .envrc)" && develle-access list
```
...
- **`upload` has NO origin backstop — Access is its ONLY auth.** The Caddy `basic_auth` that used to double-prompt is gone (verified from outside 2026-08-25: anonymous WebDAV PUT/DELETE and tusd uploads all succeed when ungated; the Caddyfile comments still claiming basic_auth are stale). `public upload` means fully anonymous read/write — keep any open window short and re-gate after.
