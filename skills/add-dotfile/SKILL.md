...
**Before choosing where the source lives, read the file.** The registry is public; a `main` may point anywhere. A source that names a person, an employer, a domain, a machine or an address — or that somebody else wrote and Louis is merely running — belongs in the PRIVATE repo: put the real copy under `~/42/GroundControl/dotfiles/<name>/` and register it with an explicit `--main='~/42/GroundControl/dotfiles/<name>/<file>'` (`~/`-prefixed, so each device resolves its own home). `claude_md` and `claude_settings` already work that way; only the entry itself lands in the public registry. A generic config Louis authored stays in `~/42/SpaceSuit/dotfiles/` as usual. Deciding this AFTER copying the file into SpaceSuit means rewriting history, so decide first.
